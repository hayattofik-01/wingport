...
- **Resume on reconnect.** When the gateway supports it, interrupted streams resume from the last received token rather than regenerating from scratch. `[ROADMAP — v0.1 ships retry-with-partial, resume lands next]`
